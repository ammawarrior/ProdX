...
$companyOptions = [];
$companyQuery = $conn->query("SELECT id, company FROM company ORDER BY company ASC");
...
<head>
    ...
    <style>
/* Main border color when focused */
.select2-container--default .select2-selection--single:focus,
.select2-container--default .select2-selection--single:hover {
    border: 1px solid #74D0D3;
}
...
.select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #74D0D3 transparent transparent transparent;
}
...
<select name="company_id" class="form-control select2" required>
    ...
    <?php foreach ($companyOptions as $company): ?>
        <option value="<?= $company['id']; ?>"><?= htmlspecialchars($company['company']); ?></option>
    <?php endforeach; ?>
</select>
